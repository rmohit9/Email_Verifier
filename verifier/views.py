...
# Local Imports
from .models import (
    VerificationJob, EmailResult, EmailCampaign, 
    CampaignRecipient, CampaignLog
)
from .serializers import (
    VerificationJobSerializer, EmailResultSerializer, 
    SingleVerifySerializer, BulkVerifySerializer,
    EmailCampaignSerializer, EmailCampaignCreateSerializer,
    CampaignRecipientSerializer, CampaignLogSerializer
)
...
# ============================================================================
# 1. HELPER CLASSES (Brevo API)
# ============================================================================

class BrevoAPIClient:
    """
    Lightweight in-file Brevo client
    """
    ...
    def __init__(self, api_key: str = None):
        ...
        if not self.api_key:
            # Fallback to django settings if env var not directly found
...
# ============================================================================
# 2. VERIFICATION PIPELINE (Pure DNS)
# ============================================================================
...
# ============================================================================
# 4. VIEW LOGIC (Standard Views & API)
# ============================================================================
...
class EmailCampaignViewSet(viewsets.ModelViewSet):
    ...
    def send(self, request, pk=None):
        """
        Trigger the asynchronous campaign sending task
        """
        ...
        # Check recipients
        pending_count = CampaignRecipient.objects.filter(campaign=campaign, status='pending').count()
        if pending_count == 0:
            return Response(
        ...
        # Trigger Celery Task
        send_campaign_emails.delay(str(campaign.campaign_id))
        
        return Response({
            'status': 'sending',
            'message': f'Campaign queued for sending to {pending_count} recipients.'
        })
